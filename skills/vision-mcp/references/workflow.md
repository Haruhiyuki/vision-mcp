...
| 「帮我建一份 X 应用的地图」/「探索一下这个 app」 | **建图驱动** | §3 系统建图 |

**判别要点**：
- 任务驱动有**明确终态**（"播了" / "写完了" / "看到答案"）
- 建图驱动是**开放式探索**（"看看有什么" / "建好待后续用"）

不确定时按任务驱动办；agent 在路径上发现"map 严重不足"应主动告诉用户"是否切到建图模式？"

## 2. 任务驱动：路径上按需混合

> **不预先判断 map 是否完整**，直接试，遇缺失当场补。
...
✅ commit_state 时把这页**几个明显的关键 control 一起**写入 baseline（不只写任务必须那一个）
✅ 发现 sidebar 还有别的导航项，顺带在 region.controls 加几个
✅ 把 OCR 关键字写入 state.anchors 让下次 detect_state 更准

❌ 为"看其他元素"专门多 snapshot 几次 → 那是建图驱动
