...
export class MacosPlatformAdapter implements PlatformAdapter {
  ...
  async ensureVirtualDisplay(opts: EnsureDisplayOptions): Promise<DisplayInfo> {
    // macOS 不创建系统虚拟显示器；如果调用方坚持 same_session_virtual_display，
    // helper 应直接拒绝。runtime 会回退到 fallbacks（real_window / existing_display）。
    return this.bridge.request<DisplayInfo>("capsule.ensure_workspace_display", {
      mode: opts.mode,
      geometry: opts.geometry,
      fallbacks: opts.fallbacks ?? ["real_window", "existing_display"],
    });
  }
