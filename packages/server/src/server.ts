    ...
    },
    {
      ...
      instructions:
        ...
        [
          ...
          "省 token（读一张整窗图 ≈ 1k+ token 且常驻上下文，优先走文本/反馈通道）：",
          "  - 观察用 vision_map.snapshot(include_image=false)：AX candidates + OCR 文本即可定位/确认，零图像成本；局部加 region_norm。",
          "  - 动作（click_at/type_text/press_key/scroll）自带 content_changed 与落点元素反馈——动作后不必截图确认。",
          "  - 真要看图：capsule.capture 配 region_norm 只截局部；确认类对比配 only_if_changed=true，内容没变不产新图。",
