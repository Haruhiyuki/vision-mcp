...
> 阅读优先级：先读 1～4，再按需查阅 `references/`。

## 1. 概念速览

- **Capsule**：受控视觉工作区。把目标窗口稳定在固定显示器/客户区，使后续动作可用归一化坐标。
- **vision-mcp.yaml**：state graph + control + workflow + 修复策略；是 agent 的唯一“地图”。
- **action_id**：`state_id.control_id[:action_type]`，agent 不要直接传屏幕坐标。
...
3. `capsule.ensure_display` → `capsule.attach_window` → `capsule.migrate_window`。
4. `capsule.validate_geometry`：违规则先调用 `vision_map.repair_minimal`，仍失败则停止并提示用户。
5. `vision_map.detect_state` → `vision_map.list_actions` → `vision_map.perform_action`（或 `run_workflow`）。
6. 出错时观察 `events[].kind`：`postcondition_failed` / `action_failed` → `repair_minimal` → 重试一次；仍失败则交还用户。
...
- `references/platform-windows.md`：Windows 适配器与权限要求。
- `references/platform-macos.md`：macOS 权限与降级方案。
